...
supervised_FT_logger = LoggerSetup(logger_name = "supervised_fine_tuning.py", log_filename_prefix = "supervised_fine_tuning").get_logger()
...
class SupervisedFineTuning:
    ...
    def __init__(self, 
        ...
        try:
            ...
            self.model.train()
            supervised_FT_logger.info("Model is set to training mode.")

            for param in self.model.parameters():
                param.requires_grad = True
                
                supervised_FT_logger.debug(f"Parameter {param} set to require gradients.")

        except Exception as e:
            supervised_FT_logger.exception(f"Error initializing SupervisedFineTuning: {repr(e)}")
    ...
    def apply_supervised_fine_tuning(self, 
        ...
        try:
            ...
            if self.model is None or self.prepared_dataset is None:
                supervised_FT_logger.error("Model or prepared dataset not loaded.")
            ...
            supervised_FT_logger.info("Starting supervised fine-tuning...")
            ...
            supervised_FT_logger.info(f"Fine-tuning started at: {total_start_time.strftime('%H:%M:%S')}")
            
            supervised_FT_logger.info("Training model...")
            
            for epoch in range(1, num_epochs + 1):
                epoch_start_time = datetime.now()
            
                supervised_FT_logger.info(f"\nEpoch {epoch}/{num_epochs} started at {epoch_start_time.strftime('%H:%M:%S')}")
                ...
                supervised_FT_logger.info(f"Epoch {epoch} finished at {epoch_end_time.strftime('%H:%M:%S')} | Time Taken: {str(time_taken)}")
            
            total_end_time     = datetime.now()
            total_time_taken   = total_end_time - total_start_time
            
            supervised_FT_logger.info("\nSupervised Fine-tuning Completed!")
            supervised_FT_logger.info(f"Total Training Time: {str(total_time_taken)}")
            supervised_FT_logger.info(f"Started at: {total_start_time.strftime('%H:%M:%S')}")
            supervised_FT_logger.info(f"Finished at: {total_end_time.strftime('%H:%M:%S')}")
            
            self.model.save_pretrained(output_dir)
            self.tokenizer.save_pretrained(output_dir)
            
            supervised_FT_logger.info(f"Model saved to: {output_dir}")
            
            return self.model, self.tokenizer
        
        except Exception as e:
            supervised_FT_logger.exception(f"Error during supervised fine-tuning: {repr(e)}")
